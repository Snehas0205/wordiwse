...
def dashboard():
    if 'user_id' in session:
        ...
        return render_template('dashboard.html', username=user.username, streak=user.streak, xp=user.xp, badges=3) 
    return redirect(url_for('login'))
...
def get_response():
    ...
    default_response = "I'm not sure how to respond to that. Can you try asking in a different way?"

    response = None
    for keyword, res in responses.items():
